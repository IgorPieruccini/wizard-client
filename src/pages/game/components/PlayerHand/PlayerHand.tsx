...
export const PlayerHand = () => {
  ...
  const pickable = useMemo(() => {
    return (
      gameState?.playersTurn === socket.id &&
      gameState.matchPhase === MatchPhase.PLAY_CARDS
    );
  }, [gameState, socket]);

  const onPick = (card: CardType) => {
    mainSocket.emit(SocketEventTypes.PICK_CARD, {
      ...card,
      playerId: socket.id,
    });
  };
    ...
    <HandContainer>
      {player?.hand.map((card) => (
        <div key={card.id}>
          ...
          {pickable && (
            <PickButton onClick={() => onPick(card)}>Pick</PickButton>
          )}
        </div>
      ))}
    </HandContainer>
