...
const mainSocket = {
  emit: (event: SocketEventTypes, data: any) => {
    /** placeholder */
  },
};
...
describe('<PlayerHand/>', () => {
  it('Display all cards', () => {
    ...
    expect(screen.getByTestId('card-yellow6')).toHaveStyle(
      `background-color: ${cardColors.yellow}`,
    );

    expect(screen.getByText('10')).toBeInTheDocument();
    expect(screen.getByTestId('card-red10')).toHaveStyle(
      `background-color: ${cardColors.red}`,
    );

    expect(screen.getByText('2')).toBeInTheDocument();
    expect(screen.getByTestId('card-blue2')).toHaveStyle(
      `background-color: ${cardColors.blue}`,
    );
  });
