...
describe('<PlayersLobby/>', () => {
  it('Display all players', async () => {
    ...
    expect(
      screen.queryByText(defaultGameStateMock.players[0].name),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText(defaultGameStateMock.players[1].name),
    ).toBeInTheDocument();
    expect(
      screen.getByText(defaultGameStateMock.players[2].name),
    ).toBeInTheDocument();
    expect(
      screen.getByText(defaultGameStateMock.players[3].name),
    ).toBeInTheDocument();
  });
});
