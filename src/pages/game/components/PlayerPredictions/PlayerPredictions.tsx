...
export const PlayerPredictions = () => {
  ...
  const hand = useMemo(() => {
    return (
      gameState?.players.find((player) => player.id === socket.id)?.hand || []
    );
  ...
  const predict = (wins: number) => {
    mainSocket.emit(SocketEventTypes.PREDICT_WIN, {
      playerId: socket.id,
      wins,
    });
  };

  if (!isCurrentPlayerTurn || gameState.matchPhase !== MatchPhase.PREDICT)
    return null;
  ...
  return (
    <div>
      ...
      <PlayerPredictionsContainer>
        <PredictButton onClick={() => predict(0)}>
          <ButtonText>0</ButtonText>
        </PredictButton>
        {hand.map((_, i) => (
          <PredictButton key={i + 1} onClick={() => predict(i + 1)}>
